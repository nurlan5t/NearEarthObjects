...
from a comparator (from the `operator` module), a reference value,
...
class AttributeFilter:
    ...
    def get(cls, approach):
        ...
        :param approach: A `CloseApproach` on which to
...
def create_filters(date=None, start_date=None, end_date=None,
    ...
    # ELABORATE:
    # Create a list (main_filter) of all necessary filter
    # functions that should be returned
    # as a tuple which represents a collection of filters
    # to be used with 'query'
    # Pass only those filter functions as a collection if & only
    # if a user-defined criteria
    # is specified as an argument with 'query'
    # If a particluar argument is not defined by the user & is
    # defaulted as None, then
    # this argument will not pass any filter function to the main_filter list
    main_filter = []
    ''' ELABORATE: Filter returns true if closed
    approach occurs on the exact "date" '''
    if(date is not None):
        main_filter.append(DateFilter(operator.eq, date))

    ''' ELABORATE: Filter returns true if closed
    approach occurs on/after the "start_date" '''
    if(start_date is not None):
        main_filter.append(DateFilter(operator.ge, start_date))

    ''' ELABORATE: Filter returns true if closed approach
    occurs on/before the "end_date" '''
    if(end_date is not None):
        main_filter.append(DateFilter(operator.le, end_date))

    ''' ELABORATE: Filter returns true if closed approach's
    approach distance is >= distance_min '''
    if(distance_min is not None):
        main_filter.append(DistanceFilter(operator.ge, distance_min))

    ''' ELABORATE: Filter returns true if closed approach's approach
    distance is <= distance_max '''
    if(distance_max is not None):
        main_filter.append(DistanceFilter(operator.le, distance_max))

    ''' ELABORATE: Filter returns true if closed approach's
    relative approach velocity is >= velocity_min '''
    if(velocity_min is not None):
        main_filter.append(VelocityFilter(operator.ge, velocity_min))

    ''' ELABORATE: Filter returns true if closed approach's relative
    approach velocity is <= velocity_max '''
    if(velocity_max is not None):
        main_filter.append(VelocityFilter(operator.le, velocity_max))

    ''' ELABORATE: Filter returns true if closed approach's
     diameter is >= diameter_min '''
    if(diameter_min is not None):
        main_filter.append(DiameterFilter(operator.ge, diameter_min))

    ''' ELABORATE: Filter returns true if closed approach's
    diameter is <= diameter_max '''
    if(diameter_max is not None):
        main_filter.append(DiameterFilter(operator.le, diameter_max))

    ''' ELABORATE: Filter returns true if closed approach's hazardous
    status matches with user-defined "hazardous" argument '''
